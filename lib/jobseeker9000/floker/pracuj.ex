defmodule Jobseeker9000.Floker.Pracuj do
  ...
  defp get_results(%__MODULE__{state: :error} = pracuj), do: pracuj
  defp get_results(%__MODULE__{state: :ok, full_html: html} = pracuj) do
    case Floki.find(html, "div#results") do
      [] -> %{pracuj | state: :error, error_text: "No div with class 'results' found."}
      div -> %{pracuj | full_html: div}
    end
  end

  defp find_lis(%__MODULE__{state: :error} = pracuj), do: pracuj
  defp find_lis(%__MODULE__{state: :ok, full_html: html} = pracuj) do
    %{ pracuj | all_the_lis: Floki.find(html, "li.results__list-container-item") }
  end

  defp test_on_first(%__MODULE__{state: :error} = pracuj), do: pracuj
  defp test_on_first(%__MODULE__{state: :ok, all_the_lis: []} = pracuj), do: pracuj
  defp test_on_first(%__MODULE__{state: :ok, all_the_lis: [h | _]} = pracuj) do
    %{pracuj | all_the_lis: [h]}
  end

  defp li_crawler(%__MODULE__{state: :error} = pracuj), do: pracuj
  defp li_crawler(%__MODULE__{state: :ok, all_the_lis: []} = pracuj), do: pracuj
  defp li_crawler(%__MODULE__{state: :ok, all_the_lis: [head | tail], results: results} = pracuj) do
  ...
  defp full_link_from_relative(%{error: nil, relative_link: relative_link} = values) do
    %{values | full_link: "https://www.pracuj.pl" <> relative_link}
  end
...
<body><div id="results" class="example"><li class="results__list-container-item"><a class="lol" href="abba">abba</a></li><li class="results__list-container-item"><a href="ojcze">ojcze</a></li></div></body>
    """
    %__MODULE__{state: :ok, full_html: html}
    |> get_results()
    |> find_lis()
    |> test_on_first()
    |> li_crawler()
